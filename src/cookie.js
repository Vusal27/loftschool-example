...
filterNameInput.addEventListener('keyup', function () {
    ...
    if (subString !== '') {
		let obj = cookiesObj(document.cookie);
...
// Cookie в объект
function cookiesObj(param) {
	newObj = param.split('; ').reduce((prev, current) => {
...
function filterTemplate(param) {
	const filtertemplate = `<td>${param.name}</td><td>${param.value}</td><td><button id='del-button' onclick="delCookie('${param.name}')">Удалить</button></td>`
...
addButton.addEventListener('click', () => {
    ...
    if (addNameInput.value !== '' && addValueInput.value !== '') {
		...
		if (filterNameInput.value === '') {
			...
			if (!filterCookie(addNameInput.value, filterNameInput.value)) {
				clearField();
			}
			if (filterCookie(addNameInput.value, filterNameInput.value) && !filterCookie(addValueInput.value, filterNameInput.value)) {
				...
				listTable.innerHTML = '';
				for (let cookie of filtered) {
					if (addNameInput.value === cookie.name && addValueInput.value !== cookie.value) {
						const filtertemplate = `<td>${cookie.name}</td><td></td><td><button id='del-button' onclick="delCookie('${cookie.name}')">Удалить</button></td>`
...
// Очистка полей в форме добавить cookie
function clearField() {
	addNameInput.value = '';
	addValueInput.value = '';
}
...
function loaderCookies() {
    ...
    for (let item of arr) {
		...
		const template = `<td>${name}</td><td>${value}</td><td><button id='del-button' onclick="delCookie('${name}')">Удалить</button></td>`
